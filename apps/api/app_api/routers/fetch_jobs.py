...
@router.post("")
def run_fetch_job(payload: FetchJobRequest) -> dict:
    source = (payload.source or get_settings().review_source_mode).lower()
    if payload.dry_run:
        result = FetchService().dry_run_location(payload.location_id)
        ...
        result = FetchService().fetch_location(payload.location_id)
    return to_jsonable(result)


@router.post("/all-active")
def run_fetch_all_active(payload: FetchAllActiveRequest | None = None) -> dict:
